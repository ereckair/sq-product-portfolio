...
(function () {
  ...
  buildMarqueeTrack(document.getElementById('category-marquee-reverse'), [...categoryNames, 'OneAshley', 'Daedalus'], 'logo-item--accent');
    ...
    if (cardsEl && initiatives.items) {
      cardsEl.innerHTML = initiatives.items
        ...
        <div class="p-6 rounded-xl border border-surface-border bg-surface-raised" data-animate-child style="--i: ${i}">
          ...
          <ul class="space-y-2">${item.highlights.map((h) => `<li class="text-xs text-zinc-500 flex gap-2"><span class="text-accent">—</span>${h}</li>`).join('')}</ul>
        </div>`
        )
        .join('');
    }
